...
then
    ...
    for seed in 0 1 2
    do
        $common_cmd="-J $exp_name ~/bin/bluemoon.sh erl python run.py --exp_name=$exp_name --n_epochs=4 --rnn_sequence_length=16 --rnn_move_window_step=1 --vec_normalize --env_id=HopperBulletEnv-v0 --total_timesteps=1e7 --num_rnns=1 --seed=$seed"
        sbatch $common_cmd --sde 
        sbatch $common_cmd
    done
fi
