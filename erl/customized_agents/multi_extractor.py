        ...
        for i, modules in enumerate(self.ensembled_rnns):
            if self.current_status==ModuleStatus.ROLLOUT:
                if new_start is not None:
                    self.hx_rollout[:,i] = (1-new_start) * self.hx_rollout[:,i]
                    self.cx_rollout[:,i] = (1-new_start) * self.cx_rollout[:,i]
                ...
                if new_start is not None:
                    self.hx_manual[i] = (1 - new_start) * self.hx_manual[i]
                    self.cx_manual[i] = (1 - new_start) * self.cx_manual[i]
                ...
                if new_start is not None:
                    self.hx_test[:,i] = (1-new_start) * self.hx_test[:,i]
                    self.cx_test[:,i] = (1-new_start) * self.cx_test[:,i]
